...
describe('Orders API Integration Tests (Controller-level)', () => {
	...
	describe('POST /api/orders', () => {
        ...
        it('should return 404 if user not found', async () => {
            ...
            expect(mockJson).toHaveBeenCalledWith(expect.objectContaining({ message: 'User not found' }))
        })
	})
    ...
    describe('DELETE /api/orders/:id', () => {
        ...
        it('should not delete non-pending order', async () => {
            ...
            expect(mockStatus).toHaveBeenCalledWith(400)
            expect(mockJson).toHaveBeenCalledWith(
                expect.objectContaining({ message: expect.stringContaining('Can only delete pending orders') })
            )
       })
    })
})
